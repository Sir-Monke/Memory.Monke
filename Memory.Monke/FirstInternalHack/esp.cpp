...
bool ESP::IsTeamGame()
{
	//This is checking the gamemode to make sure there are no game modes that have idk... no players?
	if ((*gameMode == 0 || *gameMode == 4 || *gameMode == 5 || *gameMode == 7 || *gameMode == 11 || *gameMode == 13 ||
		*gameMode == 14 || *gameMode == 16 || *gameMode == 17 || *gameMode == 20 || *gameMode == 21))
		...
		//If the game modes are true we will 
		//      |
		//      v
		return true;

	//If the game mode is not any of these we will return 
	//            |
	//            v
	else return false;
}
...
//If localPlayer team is not the same as the ent it will return 
//              |
//              v
		return false;
//And again if localPlayer team is not the same as the ent it will return 
//              |
//              v
	else return true;
}
...
bool ESP::IsValidEnt(ent* ent)
{
	if (ent)
	{
		...
		{
//If vTable pointer is not static/ is static it will return with 
//                 |
//                 v
			return true;
		}
...
void ESP::DrawESPBox(ent* e, vec3 screen, GL::Font& font)
{
	...
	//This is checking what the ent team is and putting its color either red or green for the team
	...
	//Getting the distance between the ent and as it says the localplayer
	float dist = localPlayer->pos.Distance(e->pos);

	//We are scaling the distance between the localplayer and ent so we can draw the boxes at the corrrect size
	//Credits to @Rake <--- Pog dude
